        ...
        if (select_value == pair[0]) {
            if (select_value == "Porshe" || select_value == "Toyota") {
                if (isNaN(pair[1]*count_of_product)) {
                    ...
                    let checkbox_choose = (document.querySelectorAll
                      (".calculator-checkbox input[type=checkbox]"))[0]
                    if (checkbox_choose.checked == true) {
                        let summ_result = parseInt(count_of_product*
                                                 (pair[1] + 15000))
                        result_text.innerHTML = "Цена = " + summ_result+ "р"
                        console.log("Галочка есть")
                    }
                    else {
                        result_text.innerHTML = "Цена = "+
                        pair[1]*count_of_product  + "р"
                        console.log("Галочки нет")
                    }
                }
            }
            else if (select_value == "Chevrolet" ||
                select_value == "Lamborghini") {
                if (isNaN(pair[1]*count_of_product)) {
                    result_text.innerHTML = "Неверно введено количество"
                }
                else {
                    let radio_choose = document.querySelectorAll
                    ...
                    radio_choose.forEach(function(radio) {
                        if (radio.checked) {
                            if (radio.value == "speed") {
                                let summ_result = parseInt(count_of_product*
                                                        (pair[1]+5000))
                                result_text.innerHTML = "Цена = " + summ_result
                                    + "р"
                            }
                            else if (radio.value == "spoiler") {
                                let summ_result = count_of_product*
                                   (pair[1] + 3000)
                                result_text.innerHTML = "Цена = " +
                                   summ_result + "р"
                            }
    ...
    for (let pair of prices_and_autos) {
        if (chosen_auto === pair[0]) {
            if (chosen_auto === "Porshe" || chosen_auto === "Toyota") {
                let radio_panel = document.getElementById
                ("calculator-radio-choose")
                radio_panel.style.display = "none"
                let checkbox_panel = document.getElementById
                ("calculator-checkbox-choose")
                checkbox_panel.style.display = "block"
            }
            else if (chosen_auto == "Chevrolet" ||
                     chosen_auto == "Lamborghini") {
                let radio_panel = document.getElementById
                ("calculator-radio-choose")
